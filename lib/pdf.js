...
function buildTransactionPage(tx, logoDataURI, meta) {
  const type = tx.isIncoming ? "Potvrda uplate" : "Potvrda isplate";
  const companyName = (meta && meta.companyName) || "";
  const companyIBAN = (meta && meta.iban) || "";
  ...
  if (tx.isIncoming) {
    ...
    primateljName = companyName;
    primateljRacun = formatIBAN(companyIBAN);
    primateljPNB = parsePNB(tx.pnbPrimatelja);
  } else {
    platiteljName = companyName;
    platiteljRacun = formatIBAN(companyIBAN);
    platiteljPNB = parsePNB(tx.pnbPlatitelja);
  ...
  <div class="page">
    ...
    <div class="disclaimer">Generirano pomoću Erste Transformer (OpusLabs) — nije službeni dokument banke</div>
...
const PAGE_CSS = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .disclaimer {
    ...
    font-size: 5.5pt; color: #999; text-align: center;
  }
  .bottom-bar {
...
function buildFullHTML(transactions, rekapitulacija, logoDataURI, meta) {
  const txPages = transactions
    .map((tx) => buildTransactionPage(tx, logoDataURI, meta))
    .join("\n");
...
async function generatePDF(transactions, rekapitulacija, meta, outputPath) {
  const logoDataURI = getLogoDataURI();
  const html = buildFullHTML(transactions, rekapitulacija, logoDataURI, meta);
...
async function generateSinglePagePDF(tx, meta) {
  const logoDataURI = getLogoDataURI();
  const pageHTML = buildTransactionPage(tx, logoDataURI, meta);
...
function renderSinglePageHTML(tx, meta) {
  const logoDataURI = getLogoDataURI();
  const pageContent = buildTransactionPage(tx, logoDataURI, meta);
  return `<!DOCTYPE html>
<html lang="hr"><head><meta charset="UTF-8">
<style>${PAGE_CSS}</style>
